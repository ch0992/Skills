...
Scanio 프로젝트 세션 간 컨텍스트를 이어주는 인계 파일을 작성하는 스킬.
...
1. **현재 상태 수집** (병렬 실행):
   ...
   - `tasks/reports/` — 오늘 날짜(YYYYMMDD-*) 리포트 파일 목록 확인
   - `tasks/current.md` — 현재 작업지시서 상태 확인
...
Scanio 프로젝트 Flutter Web 관리자 페이지 작업을 이어서 진행한다.

### 프로젝트 개요

- **모바일 앱** (`lib/`) + **Web 관리자 페이지** (`admin/`) 두 개의 Flutter 프로젝트가 한 레포에 존재
- **Supabase** 백엔드 (project ref: `flanoqbbwiguqyemozit`)
- **GitHub** private org repo: `NSoft-America-Inc/scanio`
- **배포 URL**: https://nsoft-america-inc.github.io/scanio/
- **관리자 계정**: admin@scanio.com / 1234

### 기술 스택 (admin/)

- Flutter Web + Riverpod + go_router + supabase_flutter
- UI: `shadcn_ui ^0.53.3` (ShadApp.custom + MaterialApp.router Hybrid)
...
- **현재 구현된 화면**: 이번 세션에서 완료된 것 반영해 최신 상태로 업데이트
- **오늘 할 작업**: 다음 세션의 핵심 작업 1개만 명시. 구체적일수록 좋음 (파일명, 위젯명, 구현 방법 포함)
...
- **이슈 목록**: `list_issues(owner="NSoft-America-Inc", repo="scanio", state="open")`
